...
public class FileLogger implements GnssListener {
  ...
  public void startNewLog() {
    synchronized (mFileLock) {
      ...
      Date now = new Date();
      String fileName = String.format("%s_%s_%s.txt",Build.MODEL,"_RAW_", formatter.format(now));
      ...
      try {
        ...
        String fileVersion =
            mContext.getString(R.string.app_version_raw)
                + " Platform: "
  ...
  public void send() {
    ...
    emailIntent.setType("*/*");
    emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Raw-Log");
  ...
  public void writeGnssMeasurementToFile(GnssClock clock, GnssMeasurement measurement)
      throws IOException {
    String clockStream =
        String.format(
