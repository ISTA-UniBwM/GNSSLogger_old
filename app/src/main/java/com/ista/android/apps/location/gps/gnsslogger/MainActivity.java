...
public class MainActivity extends AppCompatActivity
  ...
  private static final String[] REQUIRED_PERMISSIONS = {
    Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.ACTIVITY_RECOGNITION,Manifest.permission.ACCESS_BACKGROUND_LOCATION,
          Manifest.permission.ACCESS_COARSE_LOCATION
  };
  private static final int NUMBER_OF_FRAGMENTS = 7;
  private static final int FRAGMENT_INDEX_SETTING = 0;
  private static final int FRAGMENT_INDEX_SENSOR = 1;
  private static final int FRAGMENT_INDEX_LOGGER = 2;
  private static final int FRAGMENT_INDEX_PLOT = 3;
  private static final int FRAGMENT_INDEX_CMC = 4;
  private static final int FRAGMENT_INDEX_CLOCK = 5;
  private static final int FRAGMENT_INDEX_SENSORLIST = 6;
  ...
  public class ViewPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public Fragment getItem(int position) {
      ...
      //    return mFragments[FRAGMENT_INDEX_AGNSS];
        ...
        case FRAGMENT_INDEX_SENSOR:
          return mFragments[FRAGMENT_INDEX_SENSOR];
        case FRAGMENT_INDEX_SENSORLIST:
          return mFragments[FRAGMENT_INDEX_SENSORLIST];
        case FRAGMENT_INDEX_CMC:
          return mFragments[FRAGMENT_INDEX_CMC];
        case FRAGMENT_INDEX_CLOCK:
          return mFragments[FRAGMENT_INDEX_CLOCK];
        default:
    ...
    public CharSequence getPageTitle(int position) {
      ...
      switch (position) {
       ...
       //   return getString(R.string.title_agnss).toUpperCase(locale);
        ...
        case FRAGMENT_INDEX_SENSORLIST:
          return getString(R.string.title_sensorlist).toLowerCase(locale);
        case FRAGMENT_INDEX_SENSOR:
          return getString(R.string.title_sensor).toLowerCase(locale);
        case FRAGMENT_INDEX_CMC:
          return getString(R.string.title_cmc).toLowerCase(locale);
        case FRAGMENT_INDEX_CLOCK:
         return getString(R.string.title_ClockOffset).toLowerCase(locale);
        default:
  ...
  private void setupFragments() {
    ...
    mRinexLogger = new RinexLogger(getApplicationContext());
    ...
    loggerFragment.setRetainInstance(true);
    loggerFragment.setUILogger(mUiLogger);
    loggerFragment.setFileLogger(mFileLogger);
    loggerFragment.setRinexLogger(mRinexLogger);
  ...
  //  mFragments[FRAGMENT_INDEX_MAP] = mapFragment;
    ...
    agnssFragment.setRetainInstance(true);
    ...
    plotFragment.setRetainInstance(true);
    mFragments[FRAGMENT_INDEX_PLOT] = plotFragment;
    mRealTimePositionVelocityCalculator.setPlotFragment(plotFragment);

    SensorFragment sensorFragment = new SensorFragment();
    sensorFragment.setRetainInstance(true);
    sensorFragment.setRinexLogger(mRinexLogger);
    mFragments[FRAGMENT_INDEX_SENSOR] = sensorFragment;

    SensorListFragment sensorListFragment = new SensorListFragment();
    sensorListFragment.setRetainInstance(true);
    mFragments[FRAGMENT_INDEX_SENSORLIST] = sensorListFragment;

    CodeminusCarrierFragment cmcFragment = new CodeminusCarrierFragment();
    cmcFragment.setRetainInstance(true);
    mFragments[FRAGMENT_INDEX_CMC] = cmcFragment;
    mRinexLogger.setCodeMinusCarrierFragment(cmcFragment);

    internalclockoffsetFragment internalclockoffsetFragment = new internalclockoffsetFragment();
    internalclockoffsetFragment.setRetainInstance(true);
    mFragments[FRAGMENT_INDEX_CLOCK] = internalclockoffsetFragment;
    mRinexLogger.setinternalclockoffset(internalclockoffsetFragment);
